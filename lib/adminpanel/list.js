...
List.prototype.init = function(website,router){
    ...
    router.post("/list",bp.json(),function(req,res){
        ...
        var cur = website.db.collection(self.id).find(query);
        if(req.body.sort || self.defaultSort){
            cur = cur.sort(req.body.sort || self.defaultSort);
        }
    ...
    for(var name in this.columns){
        ...
        if(column.label) columns[name].label = column.label;
