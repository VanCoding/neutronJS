...
function swap(self){
    this.__proto__ = new (this.props.page.view)(this.props).type.prototype
}
...
module.exports = React.createClass({
    ...
    render:function(){
        return Page({page:{view:this.props.view,path:this.props.path,neutronPath:this.props.neutronPath,widgets:this.props.widgets,modules:this.props.modules,data:this.props.data}})
    },
    onClick:function(e){
        if(e.target.tagName == "A"){
            if(e.target.host == location.host){
                if(location.pathname == e.target.pathname){
                }else if(this.route(e.target.pathname)){
    ...
    route:function(path){
        for(var i = 0; i < this.props.routes.length; i++){
            ...
            if(route.path.test(path)){
                ...
                this.props.dynamic = route.dynamic;
                return true;
            }
        }
    }
